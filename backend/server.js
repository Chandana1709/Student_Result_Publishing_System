...
app.put('/subjectsEdit/:subjectCode', (req, res) => {
  ...
  const { subject_name, year, semester_no, dname } = req.body;
  
  const updateSubjectQuery = 'UPDATE subject SET subject_name = ?, year = ?, semester_no = ?, dname = ? WHERE subject_code = ?';
  connection.query(
  updateSubjectQuery,
  [subject_name, year, semester_no, dname, subjectCode],
  (error, results) => {
  if (error) {
